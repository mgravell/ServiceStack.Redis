...
namespace ServiceStack.Redis
{
    ...
    internal sealed class BufferedReader : IDisposable
    {
        ...
        private ValueTask<int> ReadByteSlowAsync(CancellationToken cancellationToken)
        {
            var pending = _source.ReadAsync(_buffer, _offset = 0, _buffer.Length, cancellationToken);
        ...
        private ValueTask<int> ReadSlowAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        {
            // if they're asking for more than we deal in, just step out of the way
            if (count >= buffer.Length)
