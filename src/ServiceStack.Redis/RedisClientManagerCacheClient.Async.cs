...
namespace ServiceStack.Redis
{
    partial class RedisClientManagerCacheClient : ICacheClientAsync, IRemoveByPatternAsync, ICacheClientExtendedAsync
    {
        ...
        private ValueTask<ICacheClientAsync> GetClientAsync(in CancellationToken cancellationToken)
        {
            AssertNotReadOnly();
            return redisManager.GetReadOnlyCacheClientAsync(cancellationToken);
        }

        async ValueTask<T> ICacheClientAsync.GetAsync<T>(string key, CancellationToken cancellationToken)
        {
            await using var client = await redisManager.GetReadOnlyClientAsync(cancellationToken).ConfigureAwait(false);
